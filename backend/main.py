...
def api_import_run(request: ImportRunRequest) -> dict:
    ...
    Exists because Run/WorldBible storage is in-memory (backend/runs.py,
    backend/world_bible.py's documented ponytail limitation): a server
    restart otherwise makes a saved run's data undisplayable through the
    normal UI, even though the data itself is preserved on disk. This is
    what makes stratum-demo-and-verification.md's "pre-generate and replay"
    fallback actually usable across restarts, not just within a single
    server session.
    """
    run = create_run(request.premise)
