...
| P0-2 | **Twine-opens-in-Twine has never been verified, twice in a row.** `which tweego twine` → not found; no npm/brew tooling; a subagent check never returned a result. This is the demo's closing beat. | The single most external-facing claim in the pitch ("real file, real Twine software opens it") has literally never been checked. | **Resolved directly, no subagent.** Downloaded the real Tweego v2.1.1 macOS binary (bundled Harlowe story formats) from the official GitHub release, ran it via Rosetta against both real exports in the repo (`demo_recordings/c7c529ae8bdd/story.twee`, `demo_recordings/video_assets/1f313fa61561.twee`): **both compile with exit code 0, zero errors**, producing valid playable HTML. Structural check on the compiled output confirmed 11 and 12 well-formed passages respectively with every `[[Continue->target]]` link resolving to a real passage. The Twine 2 *visual editor* (twinery.org) specifically could not be tested **from the sandboxed Electron browser used for automated verification** — its React SPA never finished mounting past the loading spinner there (confirmed IndexedDB works, a service worker registers, but the app still doesn't render even after force-clearing the service worker — a real, diagnosed, tool-specific limitation of that verification environment, not skipped or waved away). Tweego is the official companion compiler in the Twine ecosystem, so this was already genuine evidence the export is valid Twee 3, independent of a GUI-import screenshot. **Update, 2026-07-08 (round-6 voiceover review caught this, round-7 review independently re-verified it against raw evidence rather than this write-up):** `demo_recordings/video_assets/raw_capture_twine2.mp4` (87s, existing asset used in the v5 demo cut) is real screen-recorded footage of the Twine 2 desktop/web editor successfully opening `1f313fa61561.twee` — confirmed twice, independently, by extracting frames directly (once from the raw 87s file, once specifically from the 30–42s window `assemble.py` actually cuts into the final video): the library view shows "1 Story... 12 passages," the node graph shows exactly the 10 real story passages plus `StoryTitle`/`StoryData` from that file (`seed-00-d8a188` through `scene-d0571fa3`, `grep -c "^::"` on the `.twee` file returns 14, matching), and opening `scene-d14b743e` shows the full real passage text — verbatim, including the Cathedral Spire of Saint Aelith passage — matching the `.twee` source exactly. The `.twee` export and the `.mp4` capture have file mtimes 5 minutes apart on the same day, consistent with a single real capture session. This was evidently recorded in a normal (non-sandboxed) browser session, not the Electron verification environment referenced above — the two findings aren't in conflict, they're testing different environments, but this checklist entry previously implied the GUI path was entirely unverified anywhere, which is now known to be too broad. The demo's 2:03 voiceover/caption only ever claims "Tweego-verified" (never "GUI-verified"), so no video/audio claim needs correcting — but the GUI-import status below is updated to reflect this real, additional evidence. | **DONE** (compiler-verified) / **DONE** (GUI-verified — real screen recording exists showing a successful open of the same Tweego-verified file, `raw_capture_twine2.mp4`, cross-checked frame-by-frame against the `.twee` source text and file mtimes by two independent reviewers; automated sandboxed-browser verification separately remains blocked, which is a property of that verification tool, not of Twine 2 itself) |
